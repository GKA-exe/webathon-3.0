...
const RoomRequestForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.parentContact.trim())
      ...
      newErrors.parentContact = "Contact should be 10 digits";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const newUser = { ...formData, isFirstLogin: false, room: "allotting" };

      const resposne = await axios.put(
        `${process.env.NEXT_PUBLIC_URL}/student/update`,
        newUser,
      );
      ...
      alert(
        "Room request submitted successfully! Your request is pending approval.",
      );
    } catch (error) {
